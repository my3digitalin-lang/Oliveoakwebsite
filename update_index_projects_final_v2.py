...
for i, p in enumerate(project_data[2:]):
    bottom_row += f'''
        <div class="proj-card proj-card-bot rx from-bottom" style="transition-delay:{i*0.12}s">
          <img src="{p['img']}" alt="{p['name']}" loading="lazy" decoding="async">
          <div class="proj-card-inner">
            <div class="proj-number">0{p['id']} / {p['type']}</div>
            <h3 class="proj-title">{p['name']}</h3>
...
pattern_top = r'(<div class="proj-top-row">).*?(</div>\s*<!-- Bottom three equal cards -->)'
html = re.sub(pattern_top, r'\1\n' + top_row + r'\n      \2', html, flags=re.DOTALL)

pattern_bot = r'(<div class="proj-bottom-row">).*?(</div>\s*</div>\s*</section>)'
html = re.sub(pattern_bot, r'\1\n' + bottom_row + r'\n      \2', html, flags=re.DOTALL)

with open(index_file, 'w', encoding='utf-8') as f:
    f.write(html)

print("Updated index.html projects again - explicitly targeting correct html comments")
